---
title: Sharing and Confirming
description: Learn to share the event by link, QR, or KakaoTalk, then confirm the final time and notify everyone.
---

## Share the event

To gather participants, you need to share the event. OneTime offers several ways to do that.

- **Copy link** — copy the short share link and paste it into a group chat or messenger.
- **QR code** — show the QR code or save it as an image to collect responses easily, even offline.
- **Share via KakaoTalk** — share the event straight to KakaoTalk. A preview card is sent along with it, which helps boost response rates.

![The event share screen with copy-link, QR code, and KakaoTalk share buttons side by side](/images/guide/sharing-options-en.png)

Use whichever method is easiest to get the event in front of your participants.

## Confirm the schedule

Once enough people have submitted their times and the candidates have narrowed, the organizer can **confirm the final time**. Pick a slot that works for everyone on the results screen and confirm it — the event status changes to "Confirmed" and the locked-in time is shown clearly to participants.

![The confirmation screen where a final time is selected and the "Confirm" button is pressed — the locked-in time highlighted](/images/guide/sharing-confirm-en.png)

> Tip: double-check with the [participant filter](/guide/viewing-results) that all key people are available before confirming, to avoid mistakes.

## Add it to your KakaoTalk Calendar

After confirming, you can add the event to your **KakaoTalk Calendar**. Adding it to the calendar lets participants receive reminders, so there's less chance of anyone forgetting. You can find the added event under KakaoTalk > More > Calendar.

![The confirmed event registered and shown in the KakaoTalk Calendar](/images/guide/sharing-kakao-calendar-en.png)

## Wrapping up

If you've followed along this far, you can now coordinate a meeting from start to finish with OneTime. You've learned the full flow: creating an event, sharing it, finding a time that works for everyone, and confirming it.

Want to see how OneTime works in specific situations? Check out the use cases. For now, you can start by browsing the finished example events.

- [View the team meeting example](/events/view/team-meeting)
- [View the 1:1 business meeting example](/events/view/business-one-on-one)
- [View the social gathering example](/events/view/social-gatherings)

Previous: [Viewing the Results](/guide/viewing-results)
